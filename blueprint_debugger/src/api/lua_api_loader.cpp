...
#include "blueprint_debugger/api/lua_api_loader.h"
#include <cstdio>
#include <cassert>
...
IMP_LUA_API(lua_copy);
IMP_LUA_API(lua_isinteger);
IMP_LUA_API(lua_pushinteger);
IMP_LUA_API(lua_isstring);
...
extern "C" bool SetupLuaAPI()
{
	...
	LOAD_LUA_API(lua_copy);
	LOAD_LUA_API(lua_isinteger);
	LOAD_LUA_API(lua_pushinteger);
	LOAD_LUA_API(lua_isstring);
	...
	{
		printf("[BLUEPRINT][SetupLuaAPI] version: %s, lua version: %d\n", BLUEPRINT_CORE_VERSION, luaVersion);
	}
	else
	{
		printf("[BLUEPRINT][SetupLuaAPI] version: %s, lua version: jit\n", BLUEPRINT_CORE_VERSION);
	}
	return true;
}

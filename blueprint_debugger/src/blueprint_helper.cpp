...
int metaQuery(lua_State *L)
{
	...
	auto variable = BluePrintFacade::Get().GetVariableRef(var);
...
int metaAddChild(lua_State *L)
{
	...
	auto childRef = BluePrintFacade::Get().GetVariableRef(child);
	if (childRef)
	{
...
int metaGC(lua_State *L)
{
	...
	BluePrintFacade::Get().RemoveVariableRef(var);
...
int createNode(lua_State *L)
{
	...
	BluePrintFacade::Get().AddVariableRef(variable);
...
int blueprintHelperInit(lua_State *L)
{
	lua_getglobal(L, HELPER_NAME);
	if (lua_istable(L, -1))
	{
...
// 1: table
int luaopen_blueprint_helper(lua_State *L)
{
	initialized = false;
	lua_pushglobaltable(L);

	lua_pushstring(L, "blueprintHelperInit");
	lua_pushcfunction(L, blueprintHelperInit);
	lua_rawset(L, -3);
	lua_pop(L, 1);
...
int tcpListen(struct lua_State *L)
{
	...
	const auto suc = BluePrintFacade::Get().TcpListen(L, host, static_cast<int>(port), err);
	printf("[BLUEPRINT][tcpListen] luaStateAddr:%p, host:%s, port:%lld,suc:%d\n", L, host, port, suc);
...
{
	BluePrintFacade::Get().AddLuaState(L);
	return 0;
}
...
int tcpConnect(lua_State *L)
{
	...
	luaL_checknumber(L, 2);
	const auto port = lua_tointeger(L, 2);
	const auto suc = BluePrintFacade::Get().TcpConnect(L, host, static_cast<int>(port), err);
	lua_pushboolean(L, suc);
	if (suc)
		return 1;
	lua_pushstring(L, err.c_str());
...
int pipeListen(lua_State *L)
{
	...
	const auto suc = BluePrintFacade::Get().PipeListen(L, pipeName, err);
	lua_pushboolean(L, suc);
	if (suc)
		return 1;
	lua_pushstring(L, err.c_str());
...
int pipeConnect(lua_State *L)
{
	...
	std::string err;
	const auto pipeName = lua_tostring(L, 1);
	const auto suc = BluePrintFacade::Get().PipeConnect(L, pipeName, err);
	lua_pushboolean(L, suc);
	if (suc)
		return 1;
	lua_pushstring(L, err.c_str());
...
{
	const bool suc = BluePrintFacade::Get().BreakHere(L);
	lua_pushboolean(L, suc);
	return 1;
}
...
int waitIDE(lua_State *L)
{
	...
	BluePrintFacade::Get().WaitIDE(false, timeout);
	return 0;
}
...
{
	BluePrintFacade::Get().Destroy();
	return 0;
}

int gc(lua_State *L)
{
	BluePrintFacade::Get().OnLuaStateGC(L);
	return 0;
}

void handleStateClose(lua_State *L)
{
	lua_getfield(L, LUA_REGISTRYINDEX, "__BLUEPRINT__GC__");
	...
	lua_setfield(L, LUA_REGISTRYINDEX, "__BLUEPRINT__GC__");
...
bool install_blueprint_core(struct lua_State *L)
{
	...
	if (!BluePrintFacade::Get().SetupLuaAPI())
	{
		return false;
	}
#endif
	// register helper lib
	luaopen_blueprint_helper(L);
	handleStateClose(L);
	return true;
}

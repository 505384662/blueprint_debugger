...
Transporter::Transporter(bool server) : receiveSize(0),
										readHead(true),
										running(false),
										connected(false),
										serverMode(server)
...
// void Transporter::SetHandler(std::shared_ptr<BluePrintFacade> facade) {
// 	this->facade = facade;
// }
...
{
	BluePrintFacade::Get().OnReceiveMessage(document);
}
...
void Transporter::OnDisconnect()
{
	...
	// if (facade)
	// {
	BluePrintFacade::Get().OnDisconnect();
	// }
}
...
void Transporter::OnConnect(bool suc)
{
	...
	BluePrintFacade::Get().OnConnect(suc);
}

...
#include "blueprint_debugger/api/lua_api.h"
#include <ShlObj.h>
#include <TlHelp32.h>
#include <Psapi.h>
#include "easyhook.h"
...
int lua_error_worker(lua_State *L)
{
	...
	// BluePrintFacade::Get().BreakHere(L);
	return error(L);
}
...
void LoadSymbolsRecursively(HANDLE hProcess, HMODULE hModule)
{
	...
	// skip blueprint modules
	{
		static const char *blueprintModules[] = {"blueprint_hook.dll", "EasyHook.dll"};
		std::string module_path = modulePath;
		for (const char *blueprintModuleName : blueprintModules)
		{
			if (strcmp(moduleName, blueprintModuleName) == 0)
				return;
		}
	}

	BluePrintFacade::Get().SendLog(LogType::Info, "analyze: %s", moduleName);
		...
		PE_FOREACH_EXPORTED_SYMBOL(&pe, pSymbol)
		{
			if (PE_SYMBOL_HAS_NAME(pSymbol))
			{
				...
				if (name[0] == 'l' && name[1] == 'u' && name[2] == 'a')
				{
					...
					BluePrintFacade::Get().SendLog(LogType::Info, "\t[B]Lua symbol: %s", name);
				}
			}
		}
	}
...
int StartupHookMode(void *lpParam)
{
	...
	BluePrintFacade::Get().StartupHookMode(pid);

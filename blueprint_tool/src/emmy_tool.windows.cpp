			...
			if (debugEvent.dwDebugEventCode == EXCEPTION_DEBUG_EVENT)
			{
					...
					debugEvent.u.Exception.ExceptionRecord.ExceptionCode == EXCEPTION_BREAKPOINT)
				{
					...
					else
					{
						...
						std::cout << "unhandled C breakPoint at: " << (uint64_t)debugEvent.u.Exception.ExceptionRecord.ExceptionAddress << std::endl;
					}
				}
				else
				{
					...
					if (debugEvent.u.Exception.dwFirstChance == 0)
					{
						...
						case EXCEPTION_ACCESS_VIOLATION:
							std::cout << "The thread tried to read from or write to a virtual address for which it does not have the appropriate access."
									  << std::endl;
							break;
						case EXCEPTION_ARRAY_BOUNDS_EXCEEDED:
							std::cout << "The thread tried to access an array element that is out of bounds and the underlying hardware supports bounds checking."
									  << std::endl;
							break;
						...
						case EXCEPTION_DATATYPE_MISALIGNMENT:
							std::cout << "The thread tried to read or write data that is misaligned on hardware that does not provide alignment. For example, 16-bit values must be aligned on 2-byte boundaries; 32-bit values on 4-byte boundaries, and so on."
									  << std::endl;
							break;
						case EXCEPTION_FLT_DENORMAL_OPERAND:
							std::cout << "One of the operands in a floating-point operation is denormal. A denormal value is one that is too small to represent as a standard floating-point value."
									  << std::endl;
							break;
						case EXCEPTION_FLT_DIVIDE_BY_ZERO:
							std::cout << "The thread tried to divide a floating-point value by a floating-point divisor of zero."
									  << std::endl;
							break;
						case EXCEPTION_FLT_INEXACT_RESULT:
							std::cout << "The result of a floating-point operation cannot be represented exactly as a decimal fraction."
									  << std::endl;
							break;
						...
						case EXCEPTION_FLT_OVERFLOW:
							std::cout << "The exponent of a floating-point operation is greater than the magnitude allowed by the corresponding type."
									  << std::endl;
							break;
						...
						case EXCEPTION_FLT_UNDERFLOW:
							std::cout << "The exponent of a floating-point operation is less than the magnitude allowed by the corresponding type."
									  << std::endl;
							break;
						...
						case EXCEPTION_IN_PAGE_ERROR:
							std::cout << "The thread tried to access a page that was not present, and the system was unable to load the page. For example, this exception might occur if a network connection is lost while running a program over the network."
									  << std::endl;
							break;
						...
						case EXCEPTION_INT_OVERFLOW:
							std::cout << "The result of an integer operation caused a carry out of the most significant bit of the result."
									  << std::endl;
							break;
						case EXCEPTION_INVALID_DISPOSITION:
							std::cout << "An exception handler returned an invalid disposition to the exception dispatcher. Programmers using a high-level language such as C should never encounter this exception."
									  << std::endl;
							break;
						...
						case EXCEPTION_PRIV_INSTRUCTION:
							std::cout << "The thread tried to execute an instruction whose operation is not allowed in the current machine mode."
									  << std::endl;
							break;
						case EXCEPTION_SINGLE_STEP:
							std::cout << "A trace trap or other single-instruction mechanism signaled that one instruction has been executed."
									  << std::endl;
							break;
						default:
...
void ReceiveLog(DWORD processId)
{
	...
	// 1024 - 65535
	while (debugPort > 0xffff)
		debugPort -= 0xffff;
	while (debugPort < 0x400)
		debugPort += 0x400;
	debugPort++;

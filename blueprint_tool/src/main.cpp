...
{
	// printf("blueprint_tool.exe run_and_attach -dir c:/xx -dll blueprint_hook.dll -work d:fff/ -exe c:/lua/lua.exe -args test.lua");
...
int main(int argc, char **argv)
{
	...
	if (target == "attach")
	{
		// blueprint_tool.exe attach -p 100 -dir c:/xx -dll blueprint_hook.dll
		return doAttach(commandLine);
	}
	if (target == "list_processes")
	{
		// blueprint_tool.exe list_processes
		return doListProcesses();
	}
	if (target == "arch_file")
	{
		// blueprint_tool.exe arch_file FILE_PATH
		return doArchFile(commandLine);
	}
	if (target == "arch_pid")
	{
		// blueprint_tool.exe arch_pid PROCESS_ID
		return doArchPid(commandLine);
	}
	if (target == "run_and_attach")
	{
		// blueprint_tool.exe run_and_attach -dir c:/xx -dll blueprint_hook.dll -work d:fff/ -exe c:/lua/lua.exe -args test.lua

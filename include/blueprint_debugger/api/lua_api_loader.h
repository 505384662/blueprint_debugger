...
#define LOAD_LUA_API(FN)               \
	FN = (dll_##FN)LoadAPI("" #FN ""); \
	if (FN == nullptr)                 \
	return false
...
typedef void (*dll_lua_copy)(lua_State *L, int fromidx, int toidx);
DEF_LUA_API(lua_copy);
typedef int (*dll_lua_isinteger)(lua_State *L, int idx);
DEF_LUA_API(lua_isinteger);
typedef void (*dll_lua_pushinteger)(lua_State *L, lua_Integer n);
DEF_LUA_API(lua_pushinteger);
typedef int (*dll_lua_isstring)(lua_State *L, int idx);
DEF_LUA_API(lua_isstring);

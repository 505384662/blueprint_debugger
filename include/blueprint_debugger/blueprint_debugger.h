...
{
public:
	Debugger(lua_State *L, std::shared_ptr<BluePrintDebuggerManager> manager);
	~Debugger();
	...
	void HandleBreak();
	bool FindAndPushVal(lua_State *L, std::string param, int level);
	int GetInputVal(std::string inputstr, std::map<std::string, int> &inputval, std::string &leftstr);
	int ProcessSetVariableCommand(lua_State *L, const int currentFrameId, const std::string commandData);
	void SetValueByCommand(const int currentFrameId, const std::string commandData);
	...
	std::shared_ptr<BluePrintDebuggerManager> GetBluePrintDebuggerManager();
...
private:
	...
	std::shared_ptr<BluePrintDebuggerManager> manager;
	...
	bool bLoadSetVarString; //设置值时候是否在loadString
	bool bLoadGetvarString; //获取值时候是否在loadString

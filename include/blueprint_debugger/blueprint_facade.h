...
public:
	...
	std::shared_ptr<BluePrintDebuggerManager> GetDebugManager() const;
...
private:
	...
	bool isnum(string s);
	void SendVariableRsp(bool success, string name, string type, string value, string variablesReference, string tip, bool findVar);
	void OnVariableReq(const rapidjson::Document &document);
	...
	std::shared_ptr<BluePrintDebuggerManager> blueprintDebuggerManager;
